...
_admin.ping = (callback) => {
  ESClient.ping({
    ...
    if (error)
    console.log('elasticsearch cluster is down!', JSON.stringify(error));
    else if (result)
    console.log('elasticsearch cluster response!', JSON.stringify(result));
    callback();
  });
}
...
_admin.catIndices = (callback) => {
  ESClient.indices.stats({
    ...
    err && console.log('elasticsearch cluster is down!', err);
    callback();
  });
}
